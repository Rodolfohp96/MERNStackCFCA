...
import { es } from 'date-fns/locale'; // Importa el idioma español para el formato
...
const ReporteDashboard = () => {
  ...
  useEffect(() => {
    const filterData = () => {
      ...
      if (filterDateRange === "Diario") {
        ...
        startDate = subDays(now, 15); // Hace 15 días
      } else if (filterDateRange === "Mensual") {
        startDate = subDays(now, 30); // Hace 30 días
      }

      if (startDate) {
        filtered = filtered.filter((receipt) => {
          const paymentDateLocal = new Date(receipt.paymentDate);
          // Comparar solo la fecha sin hora
          return isSameDay(paymentDateLocal, now) || paymentDateLocal >= startDate;
        });
      }
  ...
  const handleDownloadAnnualReport = async () => {
    ...
    try {
      ...
      const colWidth = 7; // Número de columnas para cada bloque de mes
      const headerTitles = [
        "Alumno",
        "Grupo",
        "Grado",
      ...
      // Ordenar los recibos por grado y luego por grupo
      ...
      // Agrupar recibos por grado y mes
      ...
      let currentRow = 1; // Comienza después de las filas del encabezado

      for (const grade in groupedByGrade) {
        let startCol = 1; // Restablece la columna inicial para cada grado

        worksheet.mergeCells(`A${currentRow}:${String.fromCharCode(65 + (colWidth - 1))}${currentRow}`);
        ...
        currentRow += 1; // Avanza a la siguiente fila después del título
        ...
        for (const month in groupedByGrade[grade]) {
          ...
          // Fusionar celdas para el título del mes
          worksheet.mergeCells(`${startCell}${currentRow}:${endCell}${currentRow}`);
          const monthCell = worksheet.getCell(`${startCell}${currentRow}`);
          monthCell.value = `Mes: ${month}`;
          monthCell.alignment = { horizontal: "center", vertical: "middle" };
          monthCell.font = { bold: true, color: { argb: "FFFFFFFF" }, size: 12 };
          monthCell.fill = { type: "pattern", pattern: "solid", fgColor: { argb: "FFA10000" } };
          monthCell.border = { top: { style: "thin" }, bottom: { style: "thin" }, left: { style: "thin" }, right: { style: "thin" } };
          ...
          groupedByGrade[grade][month].forEach((receipt, rowIndex) => {
            ...
            row.getCell(startCol + 3).value = `$${receipt.amountPaid.toFixed(2)}`;
          ...
          currentRow += groupedByGrade[grade][month].length - 3; // Avanza a la siguiente fila de grupo para el siguiente mes
          startCol += colWidth + 1; // Avanza al siguiente bloque de columnas para el siguiente mes
        }

        currentRow += 4; // Añade espacio después de cada sección de grado
      }
          ...
          <TableBody>
            {filteredReceipts.map((receipt) => (
              <TableRow key={receipt._id}>
                ...
                <TableCell sx={{ textAlign: "center", border: "2px solid #224573" }}>{format(parseISO(receipt.paymentDate), 'yyyy-MM-dd', { locale: es })}</TableCell>
