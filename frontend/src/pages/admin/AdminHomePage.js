import { TableContainer, Container, Grid, Paper, TextField, List, ListItem, ListItemText, Button, Table, TableBody, TableCell, TableHead, TableRow, MenuItem, Select, InputLabel, FormControl, Typography } from '@mui/material'; 
...
const AdminHomePage = () => {
    ...
    useEffect(() => {
        ...
        dispatch(fetchAllReceipts(adminID));
    }, [adminID, dispatch]);

    useEffect(() => {
        if (receiptsList && receiptsList.length > 0) {
            const today = new Date().toISOString().split('T')[0]; 
            const todayReceipts = receiptsList.filter(receipt => receipt.paymentDate.startsWith(today));
            setFilteredReceipts(todayReceipts);

            const totalAmountPaid = receiptsList.reduce((sum, receipt) => sum + receipt.amountPaid, 0);
                            ...
                            <List sx={{ mt: 2 }}>
                                {filteredStudents.length > 0 ? (
                                    filteredStudents.map((student) => (
                                        <ListItem key={student._id} sx={{ display: 'flex', justifyContent: 'space-between' }}>
                                            <ListItemText
                                            ...
                                            <Button
                                                variant="contained"
                                                color="primary"
                                                onClick={() => navigate(`/Admin/students/student/tuitions/${student._id}`)}
                                                sx={{ mr: 2 }}
                                            >
                                                Colegiaturas
                                            </Button>
                                            <Button
                                                variant="contained"
                                                color="secondary"
                                                onClick={() => navigate(`/Admin/students/student/receipts/${student._id}`)}
                                            >
                                                Recibos
                                            </Button>
                                        </ListItem>
